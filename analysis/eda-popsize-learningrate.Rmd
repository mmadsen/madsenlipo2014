...
qplot(orbit_number, data=saa_finalized_combined, geom="density", binwidth=2) + facet_grid(learning_rate ~ population_size)
```
...
qplot(log(autgroupsize), data=saa_finalized_combined, geom="density", binwidth=2) + facet_grid(learning_rate ~ population_size)
```
...
qplot(msg_lambda, data=saa_finalized_combined, geom="density", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```
...
qplot(mem_beta, data=saa_finalized_combined, geom="density", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```
...
qplot(radius_frac, data=saa_finalized_combined, geom="density", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```

Mean degree:

```{r eda-degree, results="asis", message=FALSE, warning=FALSE}
qplot(mean_degree, data=saa_finalized_combined, geom="density", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```

Remaining density:

```{r eda-density, results="asis", message=FALSE, warning=FALSE}
qplot(remaining_density, data=saa_finalized_combined, geom="density", binwidth=0.01) + facet_grid(learning_rate ~ population_size)
```
...
qplot(mean_orbit_multiplicity, data=saa_finalized_combined, geom="density", binwidth=0.5) + facet_grid(learning_rate ~ population_size)
```
