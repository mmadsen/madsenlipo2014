# SAA-2, SAA-3, SAA-4 EDA:  Population Size and Learning Rate #
...
At this stage, I'm doing some simple histograms to see where there is variation across the paramter space.  Orbit number seems to have some variation.  With higher learning rate, we lose the mode at 1 and 2, and the distribution seems to shift to the right slightly with high learning rate and larger population size.  

```{r eda-orbit, results="asis", message=FALSE, warning=FALSE}
qplot(orbit_number, data=saa_finalized_combined, geom="histogram", binwidth=2) + facet_grid(learning_rate ~ population_size)
```
...
qplot(log(autgroupsize), data=saa_finalized_combined, geom="histogram", binwidth=2) + facet_grid(learning_rate ~ population_size)
```
...
qplot(msg_lambda, data=saa_finalized_combined, geom="histogram", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```
...
qplot(mem_beta, data=saa_finalized_combined, geom="histogram", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```
...
qplot(radius_frac, data=saa_finalized_combined, geom="histogram", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```

Mean degree:

```{r eda-degree, results="asis", message=FALSE, warning=FALSE}
qplot(mean_degree, data=saa_finalized_combined, geom="histogram", binwidth=0.05) + facet_grid(learning_rate ~ population_size)
```

Remaining density:

```{r eda-density, results="asis", message=FALSE, warning=FALSE}
qplot(remaining_density, data=saa_finalized_combined, geom="histogram", binwidth=0.01) + facet_grid(learning_rate ~ population_size)
```
...
qplot(mean_orbit_multiplicity, data=saa_finalized_combined, geom="histogram", binwidth=0.5) + facet_grid(learning_rate ~ population_size)
```
